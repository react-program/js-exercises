    ...
    .then(json => {
      ...
      for (var stk in json.data.stock) {
        ...
        stock_li +=
          ...
          ")</li>";
        if (localStorage.getItem("cart_" + json.data.stock[stk].item)) {
          cart_li +=
            "<li>" +
            json.data.stock[stk].item +
            "(" +
...
function checkItem(item, qty, type, category) {
  if (qty == 1 && type == "add" && category == "cart") {
    ...
    if (category == "stock") {
      qty = Number(qty) - 1;
      console.log(localStorage.getItem(item), "item");
      if (localStorage.getItem(item) == 0) {
        let li = document.createElement("li");
        li.appendChild(document.createTextNode(item + "(" + qty + ")"));
        document.querySelector("#" + category + " ul").appendChild(li);
      } else {
        updateCategory(item, qty, type, category);
      }
    }
    updateCategory(item, qty, type, category);
  }
...
function updateCategory(item, qty, type, category) {
  if (qty == 0) {
    qty = 1;
  }
  let cartId = document.querySelector("#" + category + " ul");
...
function updateStock(e, index) {
  ...
  for (i = 0; i < stockList.length; i++) {
    ...
    if (stockText == cartText) {
      ...
      checkItem(
        cartText,
        localStorage.getItem("cart_" + cartText),
        "remove",
        "stock"
      );
    }
